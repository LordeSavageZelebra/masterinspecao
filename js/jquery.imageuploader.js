/* Função para iniciar o upload ao clicar no botão */
(function ($) {
    $.fn.uploader = function (options, testMode) {
        return this.each(function (index) {
            options = $.extend({
                ...
                badFileTypeMessage: 'Este tipo de imagem não pode ser utilizado no upload.',
                ajaxUrl: '/ajax/upload',
                testMode: false
            }, options);
            ...
            // Manipulação dos elementos DOM do javascript
            var dom = {
                uploaderBox: $(this),
            ...
            // Limpa todo o conteúdo para receber as novas imagens
            dom.uploaderBox.empty();

            // Cria e armazena o comteudo de imagem recebido
            setupDOM(dom);

            // Manipulação dos eventos de seleção e instruções
            bindUIEvents();
            ...
            function addItem (file) {
                ...
                if (options.fileTypeWhiteList.indexOf(getExtension(file.name).toLowerCase()) !== -1) {
                    ...
                    // Se o arquivo apresentar erro, adicionar o DOM
            ...
            function selectFilesHandler (e) {
                ...
                if (!state.isUploading) {
                   var files = e.target.files || e.dataTransfer.files || e.dataTransfer.getData;
                    ...
                    // Opção para adicionar mais imagens
